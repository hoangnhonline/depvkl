						...
						@if(!empty( (array) $productList ))
							<ul>
								...
								<li class="news-new-item">
									<div class="news-new-item-head"><a href="{{ route('chi-tiet', [$product->slug_loai, $product->slug, $product->id]) }}"><img title="{{ $product->title }}" src="{{ $product->image_urlxxxs ? Helper::showImageThumb($product->image_urlxxxs) : URL::asset('public/admin/dist/img/no-image.jpg') }}" alt="{!! $product->title !!}"></a></div>
